...
# SHORT NOTE:
# How do we now at what offset n is? just send a lot of bytes, when the index changes drastically (program tells) that is where 'n' is at
# How to get the return address? this is a two step process, first, find the canary's position, after that adjust 'n' to skip 8 bytes
# then, just overflow the buffer, this part you can use find_offset() or do it manually
