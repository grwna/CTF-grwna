...
# Assign values to the register parameter  
def interpret_imm(offset, register: bytes, value):      # Immediate
    register_char = describe_register(register)
    print(f"IMM {register_char} = {value}")
    write_register(offset, register, value)

# Writes value at register 2 to offset + value at register 1 
def interpret_stm(offset, register1, register2):    # Store memory
...
# Adds value of register2 to register1
...
Extra Note: i am doubly dumb
I was right the point is to analyze how these interpreter work
"""